##################################
Building and uploading GDAL wheels
##################################
...
https://travis-ci.org/MacPython/gdal-wheels
...
* making a commit to the `gdal-wheels` repository (e.g. with `git
...
* ``-t all`` downloads / uploads built wheels at this URL for all platforms
  (macOS, manylinux, windows - if present);
...
``2.2.1`` is the version to download / upload.

In order to upload to the PyPI server, you will need something like this in
your ``~/.pypirc`` file::
...
``gdal-2.2.1-`` from http://wheels.scipy.org to ``~/wheelhouse``,
